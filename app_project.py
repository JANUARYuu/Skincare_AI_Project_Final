...
def predict_skin_condition(cropped_face_rgb):
    """
    ฟังก์ชันทำนายโทนสีผิว, ประเภทผิว และคะแนนสิว โดยใช้ Logic สีผิวที่แม่นยำขึ้น
    """
    
    avg_color = np.mean(cropped_face_rgb, axis=(0, 1))
    R, G, B = avg_color[0], avg_color[1], avg_color[2]
    
    # คำนวณค่าความสว่าง (Luminosity) สำหรับ Tone Group
    # สูตร Luma (มาตรฐาน HDTV): L = 0.2126*R + 0.7152*G + 0.0722*B
    luminosity = 0.2126*R + 0.7152*G + 0.0722*B
    
    # 1. การทำนาย Tone Group (ระดับความเข้ม)
    if luminosity > 185:
    ...
    # 2. การทำนาย Undertone (หลักการ: R/B Ratio)
    # ใช้เกณฑ์ที่เข้มงวดขึ้นในการจำแนก
    
    # Warm: ถ้า R เด่นกว่า B (บ่งชี้ถึงสีเหลือง/ทอง)
    if R > B * 1.15: 
        undertone = random.choice(['Warm-Olive', 'Warm'])
    # Cool: ถ้า B เด่นกว่า R (บ่งชี้ถึงสีชมพู/แดง)
    elif B > R * 1.15: 
        undertone = random.choice(['Cool-Pink', 'Cool'])
    # Neutral: ถ้าค่าสี R และ B ใกล้เคียงกันมาก
    else:
        undertone = 'Neutral'
    ...
    # 4. การทำนายคะแนนสิว (จำลองตามความสว่าง)
    if luminosity > 185:
        acne_score = random.choice([1, 1, 2, 2, 3]) 
    elif luminosity < 100:
        acne_score = random.choice([2, 3, 3, 4, 5]) 
    else:
        acne_score = random.choice([2, 2, 3, 3, 4]) 
...
# ----------------------------------------------------------------------
# ... (ส่วน UI และการแสดงผลพร้อมคำอธิบาย ยังคงเหมือนเดิม)
        ...
        if cropped_face is not None:
            ...
            with col2:
                ...
                if 'Cool' in undertone:
                    st.caption("ผิวมีแนวโน้มอมชมพู/แดง เหมาะกับ Foundation ที่มีเบสสีชมพู/แดง")
